    ...
    <>
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <a
            href="#home"
            className="text-2xl font-extrabold tracking-wider bg-gradient-to-r from-primary via-secondary to-accent bg-clip-text text-transparent hover:scale-105 transition-transform"
          >
            BEKSULTON
          </a>
          ...
          <nav className="hidden lg:flex items-center space-x-6">
            {navLinks.map((link) => {
              ...
              return (
                <a
                  key={link.key}
                  href={link.href}
                  className={`text-lg font-semibold transition-all duration-300 relative group py-2 ${
                    isActive 
                      ? 'text-primary' 
            ...
            <div className="relative" ref={langMenuRef}>
              <button
                ...
                className="px-3 py-2 rounded-full glass-card hover:border-primary/50 text-slate-650 dark:text-slate-300 hover:text-primary dark:hover:text-primary flex items-center space-x-1.5 transition-all text-base font-bold"
              >
                <FiGlobe className="w-3.5 h-3.5" />
                <span>{getLanguageLabel(language)}</span>
                <FiChevronDown className={`w-3 h-3 transition-transform duration-200 ${langMenuOpen ? 'rotate-180' : ''}`} />
              </button>
              ...
              <AnimatePresence>
                {langMenuOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 10 }}
                    className="absolute right-0 mt-2 py-1.5 w-24 rounded-xl glass-nav shadow-xl border border-slate-200 dark:border-white/5 flex flex-col overflow-hidden"
                  >
                    {(['en', 'uz', 'ru'] as const).map((lang) => (
                      <button
                        key={lang}
                        onClick={() => handleLanguageChange(lang)}
                        className={`px-4 py-2 text-base font-bold text-left transition-colors ${
                          language === lang
            ...
            <button
              ...
              className="p-2 rounded-full glass-card hover:border-primary/50 text-slate-650 dark:text-slate-300 hover:text-primary dark:hover:text-primary transition-all duration-300"
            ...
            <a
              href="#contact"
              className="px-5 py-2.5 rounded-full bg-gradient-to-r from-primary to-secondary text-white font-bold text-base tracking-wider uppercase hover:shadow-neon-purple hover:scale-[1.03] transition-all duration-300"
            >
              {t('nav.hireMe')}
            </a>
          </nav>
